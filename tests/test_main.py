...
def test_shelly_1_gen4() -> None:
    ...
    assert json.loads(hass.services.call.call_args_list[8][0][2]["payload"]) == {'avty': [{'pl_avail': 'true', 'pl_not_avail': 'false', 't': '~online'},
            {'t': '~status/rpc',
             'val_tpl': '{%if '
                        'value_json.mqtt.connected%}online{%else%}offline{%endif%}'}],
   'dev': {'cns': [['mac', 'aa:bb:cc:dd:ee:ff']],
           'cu': 'http://shelly1g4-aabbccddeeff.local/',
           'hw': 'gen4',
   ...
   'o': {'name': 'Shellies Discovery Gen2',
         'sw': '0.0.0',
         'url': 'https://github.com/bieniu/ha-shellies-discovery-gen2'},
   'qos': 0,
   'stat_t': '~status/switch:0',
   ...
   '~': 'shelly1g4-aabbccddeeff/'
    ...
    assert json.loads(hass.services.call.call_args_list[9][0][2]["payload"]) == {'avty': [{'pl_avail': 'true', 'pl_not_avail': 'false', 't': '~online'},
            {'t': '~status/rpc',
             'val_tpl': '{%if '
                        'value_json.mqtt.connected%}online{%else%}offline{%endif%}'}],
   'dev': {'cns': [['mac', 'aa:bb:cc:dd:ee:ff']],
   ...
   '~': 'shelly1g4-aabbccddeeff/'}
    ...
    assert json.loads(hass.services.call.call_args_list[10][0][2]["payload"]) == {'atype': 'trigger',
   'dev': {'cns': [['mac', 'aa:bb:cc:dd:ee:ff']],
           'cu': 'http://shelly1g4-aabbccddeeff.local/',
           'hw': 'gen4',
           'ids': ['aa:bb:cc:dd:ee:ff'],
   ...
   'val_tpl': '{{value_json.params.events.0.id}}_{{value_json.params.events.0.event}}'}
    ...
    assert json.loads(hass.services.call.call_args_list[11][0][2]["payload"]) == {'atype': 'trigger',
   'dev': {'cns': [['mac', 'aa:bb:cc:dd:ee:ff']],
           'cu': 'http://shelly1g4-aabbccddeeff.local/',
           'hw': 'gen4',
           'ids': ['aa:bb:cc:dd:ee:ff'],
   ...
   'val_tpl': '{{value_json.params.events.0.id}}_{{value_json.params.events.0.event}}'}
    ...
    assert json.loads(hass.services.call.call_args_list[12][0][2]["payload"]) == {'atype': 'trigger',
   'dev': {'cns': [['mac', 'aa:bb:cc:dd:ee:ff']],
           'cu': 'http://shelly1g4-aabbccddeeff.local/',
           'hw': 'gen4',
           'ids': ['aa:bb:cc:dd:ee:ff'],
